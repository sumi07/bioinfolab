...
;
float store_x[20000],store_y[20000],store_z[20000],store_x1[20000],store_y1[20000],store_z1[20000];
f1=fopen("1asy.pdb","r");
f2=fopen("about.pdb","w");
while(fgets(line,100,f1)!=NULL)
{
     ...
     for(i=0;i<c1;i++)
     {
         for(j=0;j<c2;j++)
        { 
                 find_distance(store_x[i],store_y[i],store_z[i],store_x1[j],store_y1[j],store_z1[j]);
                   
                  if(distance<=4.5)
                       {
                           
                     printf("%i  %s  %s %i %i  %s  %s %i",store_slno1[j],store_atm_nm1[j],store_res_nm1[j],store_res_no1[j],store_slno[i],store_atm_nm[i],store_res_nm[i],store_res_no[i]);
                      }
                }
         }
  }
